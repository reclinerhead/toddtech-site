...
- `publicPages` is optional and, when present, renders a "Live and open to the public" section directly under the system diagram (before Tech Stack): a heading and intro from the data, one link card per page (eyebrow, title, description, and the bare URL, opening in a new tab), and an optional small-print `sourcesNote`. It exists because Hearth's two public water quality pages (`hearth.toddtech.llc/water/kalamazoo-mi` and `/water/portage-mi`) are the only running product surface an anonymous visitor can open, so they are featured rather than buried in a caption. The copy there deliberately stresses that every finding comes from EPA records and the utility's published annual report; keep that emphasis when editing, and keep volatile numbers (violation counts, sample values) out of the static copy since the live pages update on their own.
